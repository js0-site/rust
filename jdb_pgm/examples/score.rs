...
fn main() {
  ...
  // Design: If Pass All, Score is high. If Fail, Score is low/penalty.
  // We want to differentiate between "Almost Passed" and "Totally Failed".
  // Score = TptScore + SizeScore - Penalties
  let score;
  
  if dram_pass && tpt_pass && lat_pass {
      // Bonus for exceeding expectations
      // Maximize throughput * compression
      score = 1000.0 + (pc_mops / (size_ratio * 10.0));
  } else {
      // Penalties based on distance to target
      let mut penalty = 0.0;
      if !dram_pass { penalty += (size_ratio - 0.30) * 1000.0; }
      if !tpt_pass { penalty += (0.95 - tpt_ratio) * 1000.0; }
      if !lat_pass { penalty += (lat_ratio - 1.10) * 1000.0; }
      
      // Ensure positive score for tune.py, but very low relative to pass
      score = (100.0 - penalty).max(0.001);
  }
