...
const getSystemInfo = async () => {
  ...
  if (platform === "darwin")
    os_name = `macOS ${(await $`sw_vers -productVersion`).stdout.trim()}`;
...
const genMd = async (data, accuracy_data, build_time_data, sys, lang_code) => {
  ...
  const acc_results = accuracy_data.results.filter(
      (r) => parseInt(r.data_size) === 1000000,
    ),
    acc_map = {};
  ...
  const bt_results = build_time_data.results.filter(
      (r) => parseInt(r.data_size) === 1000000,
    ),
    bt_map = {};
