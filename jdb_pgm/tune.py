...
def update_build_rs(config):
    ...
    if not os.path.exists(build_rs_path):
        print(f"Warning: {build_rs_path} not found. Skipping update.")
        return

    with open(build_rs_path, "r", encoding="utf-8") as f:
        content = f.read()

    # Define replacements mapping: config_key -> (env_var_name, rust_type_suffix)
    replacements = {
        "block_len": ("PC_BLOCK_LEN", "usize"),
        "epsilon": ("PC_EPSILON", "usize"),
        "ex_penalty": ("PC_EX_PENALTY", "u8"),
    }

    new_content = content
    for key, (env_var, suffix) in replacements.items():
        if key in config:
            val = config[key]
            # Regex to find: get_env_or_default("ENV_VAR", 123suffix)
            # We want to replace 123 with val.
            # Pattern: (get_env_or_default\s*\(\s*"{env_var}"\s*,\s*)(\d+)(?:{suffix})?(\))
            pattern = f'(get_env_or_default\\s*\\(\\s*"{env_var}"\\s*,\\s*)(\\d+)(?:{suffix})?(\\))'
            
            # Simplified pattern: match header, numeric value, suffix (optional), closing
            # get_env_or_default("PC_BLOCK_LEN", 256usize)
            regex = re.compile(f'(get_env_or_default\\s*\\(\\s*"{env_var}"\\s*,\\s*)(\\d+)({suffix})?(\\))')
            
            # Check if match exists
            if regex.search(new_content):
                new_content = regex.sub(f"\\g<1>{val}{suffix}\\4", new_content)
                print(f"  Updating {env_var} -> {val}")
            else:
                print(f"  Warning: Could not find pattern for {env_var} in build.rs")

    if new_content != content:
        with open(build_rs_path, "w", encoding="utf-8") as f:
            f.write(new_content)
        print(f"Updated {build_rs_path} successfully.")
    else:
        print("No changes needed for build.rs.")
