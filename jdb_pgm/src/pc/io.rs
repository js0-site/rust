...
/// 从字节流反序列化 Pc。
pub fn load<const B: usize>(bytes: &[u8]) -> Result<PcBase<B>> {
  let mut pos = 0;

  if bytes.len() < 8 {
    return Err(PgmError::InvalidData(
  ...
  macro_rules! check_len {
    ($needed:expr) => {
      if pos + $needed > bytes.len() {
        return Err(PgmError::InvalidData(format!(
  ...
  let block_meta = deserialize_vec!(|| -> Result<BlockMeta> {
  ...
  let residuals = deserialize_vec!(|| -> Result<u64> { Ok(read_u64!()) });

  let exceptions = deserialize_vec!(|| -> Result<u64> { Ok(read_u64!()) });

  let bitmap = deserialize_vec!(|| -> Result<u64> { Ok(read_u64!()) });
