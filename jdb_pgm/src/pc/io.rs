...
pub fn dump<const B: usize>(pc: &PcBase<B>) -> Vec<u8> {
  ...
  serialize_vec!(pc.segments, |s: &CompactSegment| {
...
pub fn load<const B: usize>(bytes: &[u8]) -> crate::error::Result<PcBase<B>> {
  ...
  let segments = deserialize_vec!(|| -> crate::error::Result<CompactSegment> {
    let start_idx = read_u32!();
  ...
  let block_meta = deserialize_vec!(|| -> crate::error::Result<BlockMeta> {
    ...
    // block_meta layout in serialized form is somewhat packed (13 bytes),
    // but here we are reading field by field which matches dump macro.
    // Wait, the previous dump code pushed `b.bit_width` (u8) directly.
    check_len!(1);
    let bit_width = bytes[pos];
    pos += 1;
    let seg_idx = u32::from_le_bytes(read_slice!(4).try_into().unwrap());
