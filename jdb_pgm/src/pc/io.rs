...
pub fn load<const B: usize>(bytes: &[u8]) -> PcBase<B> {
  ...
  // Bulk load residuals (u64 alignment might be issue if bytes not aligned?
  // But u64::from_le_bytes handles unaligned)
  // Optimization: use slice casting if aligned? No, safer to loop or unsafe cast.
  // Loop is fine for now, compiler optimizes.
