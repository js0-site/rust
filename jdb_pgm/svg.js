...
const getData = (results, dataSize, epsilon) => {
  const algos = ["jdb_pgm", "external_pgm", "hashmap", "binary_search", "btreemap"];
  return algos.map((algo) => {
    const r = results.find(
      (r) =>
        r.data_size === dataSize &&
        r.algorithm === algo &&
        (algo === "jdb_pgm" || algo === "external_pgm" ? r.epsilon === epsilon : true),
    );
    return r || null;
  });
};
...
const getAccuracyData = (results, dataSize, epsilon) => {
  const jdb = results.find(
    (r) => r.data_size === dataSize && r.algorithm === "jdb_pgm" && r.epsilon === epsilon,
  );
  const ext = results.find(
    (r) => r.data_size === dataSize && r.algorithm === "external_pgm" && r.epsilon === epsilon,
  );
  return { jdb, ext };
};
...
const genThroughputChart = (data, baseY, names, epsilon, lang) => {
  const h = CHART_H;
  const maxThroughput = Math.max(...data.filter(Boolean).map((r) => r.throughput / 1e6));
  let svg = genYAxis(baseY, h, maxThroughput, "M/s");
  svg += genYLabel(baseY, h, lang === "en" ? "Throughput (M/s)" : "吞吐量 (M/s)");
  ...
  data.forEach((r, i) => {
    if (r) {
      ...
      svg += genBar(x, baseY, h, val, maxThroughput, getColor(algos[i]), names[algos[i]], subLabel);
    }
    x += BAR_W + BAR_GAP;
  });
  return svg;
...
const genMemoryChart = (data, baseY, names, title, lang) => {
  const h = CHART_H;
  const maxMem = Math.max(...data.filter(Boolean).map((r) => r.memory_bytes / (1024 * 1024)));
  ...
  data.forEach((r, i) => {
    if (r) {
      ...
      const subLabel = r.epsilon ? `e=${r.epsilon}` : null;
      svg += genBar(x, baseY, h, val, maxMem, getColor(algos[i]), names[algos[i]], subLabel);
    }
    x += BAR_W + BAR_GAP;
  });
  return svg;
...
const genAccuracyChart = (accData, baseY, names, title, epsilon, lang) => {
  ...
  const maxError = Math.max(epsilon, jdb?.avg_error || 0, ext?.avg_error || 0) * 1.2;
  let svg = `<text x="${W / 2}" y="${baseY - 20}" fill="#333" font-size="20" font-weight="bold" text-anchor="middle">${escXml(title)}</text>`;
  ...
  const barData = [
    {
      label: lang === "en" ? `Epsilon (e=${epsilon})` : `配置精度 (e=${epsilon})`,
      val: epsilon,
      color: "#94a3b8",
    },
    {
...
const genSvg = (benchData, accuracyData, lang) => {
  ...
  const sizeLabel =
    lang === "en" ? `${formatDataSize(dataSize)} Elements` : `${formatDataSize(dataSize)} 个元素`;
  const memTitle = lang === "en" ? "Memory Usage Comparison" : "内存使用对比";
  const accTitle = lang === "en" ? "Prediction Accuracy Comparison" : "预测精度对比";
  const note =
    lang === "en"
  ...
  // 计算总高度
  const chartGap = 160;
  const totalH = M.t + CHART_H + chartGap + CHART_H + chartGap + CHART_H + M.b + 80;
