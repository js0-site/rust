...
fn main() {
  ...
  // Defaults based on tune.py results (256, 64, 8)
  // 基于 tune.py 调优结果的默认值 (256, 64, 8)
  let block_len = get_env_or_default("PC_BLOCK_LEN", 256usize);
  let epsilon = get_env_or_default("PC_EPSILON", 64usize);
  let ex_penalty = get_env_or_default("PC_EX_PENALTY", 8u8);
