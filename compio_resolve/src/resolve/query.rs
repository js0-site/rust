...
pub(crate) async fn query(resolv_conf: &ResolvConf, name: &str) -> io::Result<QueryResult> {
...
async fn query_ns_tcp(
  ...
  let BufResult(res, len_buf) = compio_runtime::time::timeout(timeout, socket.read_exact([0u8; 2]))
    .await
