...
const combine = (data, lang) => {
  ...
  const sortedByEffectiveOps = [...results].sort((a, b) => b.effective_ops - a.effective_ops);
  const sortedByOps = [...results].sort((a, b) => b.ops_per_second - a.ops_per_second);
  const sortedByHitRate = [...results].sort((a, b) => b.hit_rate - a.hit_rate);
  const sortedByMemory = [...results].sort((a, b) => a.memory_mb - b.memory_mb);
  ...
  const noteLine2 =
    lang === "en" ? `miss_latency = ${missLatencyUs}µs` : `未命中延迟 = ${missLatencyUs}µs`;
