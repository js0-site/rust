...
const bar2d = (x, y, w, h, c) => '<rect x="' + x + '" y="' + y + '" width="' + w + '" height="' + h + '" fill="' + c + '"/>';

const barChart = (results, m, yOffset, getValue, formatLabel) => {
  const cH = m.chartH - m.t - m.b,
    libs = results.map((r) => r.lib);
  ...
  const bW = Math.min(60, (m.cW - 20) / libs.length - 10),
    gap = (m.cW - libs.length * bW) / (libs.length + 1);
...
const combine = (data, lang) => {
  const { results } = data,
    libs = results.map((r) => r.lib),
    m = { t: 70, r: 40, b: 100, l: 90, cW: 570, chartH: 320 },
    W = m.l + m.cW + m.r,
    chartGap = 40,
    titleH = 90,
    mainTitle = lang === "en" ? "Rust LRU Benchmark" : "Rust LRU 评测",
    subTitle = lang === "en"
      ? "Real-world Data Distribution, Fixed Memory Budget"
      : "模拟真实世界数据分布，固定内存大小",
    titles = lang === "en"
      ? ["Hit Rate (%)", "Memory (KB)", "Equivalent OPS (M/s)", "OPS (M/s)"]
      : ["命中率 (%)", "内存 (KB)", "等价 OPS (百万/秒)", "OPS (百万/秒)"],
    sortedByEffectiveOps = [...results].sort((a, b) => b.effective_ops - a.effective_ops),
    sortedByOps = [...results].sort((a, b) => b.ops_per_second - a.ops_per_second),
    sortedByHitRate = [...results].sort((a, b) => b.hit_rate - a.hit_rate),
    sortedByMemory = [...results].sort((a, b) => a.memory_mb - b.memory_mb),
    charts = [
      {
    ...
    missLatencyUs = (data.miss_latency_ns / 1000).toFixed(1),
    noteLine1 = lang === "en"
      ? "Equivalent OPS = 1 / (avg_op_time + miss_rate × miss_latency)"
      : "等价 OPS = 1 / (平均操作时间 + 未命中率 × 未命中延迟)",
    noteLine2 = lang === "en" ? "miss_latency = " + missLatencyUs + "µs" : "未命中延迟 = " + missLatencyUs + "µs",
    noteLine3 = data.miss_latency_method,
    noteGap = 56;

  let svg = '<text x="' + (W / 2) + '" y="80" text-anchor="middle" font-size="' + MAIN_TITLE_SIZE + '" font-weight="bold" fill="#222">' + mainTitle + '</text>' +
    '<text x="' + (W / 2) + '" y="' + (80 + MAIN_TITLE_SIZE * 0.8) + '" text-anchor="middle" font-size="22" fill="#666">' + subTitle + '</text>';
  ...
  charts.forEach((chart, idx) => {
    ...
    if (idx === 2) {
      svg += '<text x="' + (W / 2) + '" y="' + (yOffset + 16) + '" text-anchor="middle" font-size="14" fill="#888">' + noteLine1 + '</text>' +
        '<text x="' + (W / 2) + '" y="' + (yOffset + 36) + '" text-anchor="middle" font-size="14" fill="#888">' + noteLine2 + '</text>' +
        '<text x="' + (W / 2) + '" y="' + (yOffset + 56) + '" text-anchor="middle" font-size="14" fill="#888">' + noteLine3 + '</text>';
      yOffset += noteGap;
    }
    yOffset += chartGap;
  });
  ...
  libs.forEach((lib, i) => {
    ...
    legend += '<rect x="' + x + '" y="' + y + '" width="20" height="20" fill="' + getColor(lib) + '"/>' +
      '<text x="' + (x + 28) + '" y="' + (y + 18) + '" font-size="18" fill="#333">' + lib + '</text>';
  });

  const legendRows = Math.ceil(libs.length / legendCols),
    H = yOffset + legendRows * 32 + 20;
...
compressAndWrite = (filename, content) => {
