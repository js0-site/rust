...
const OP_DIST = [
  {
    op: "Read",
    pct: 90,
    desc: "Cache lookups dominate",
    source: "Twitter: 99%+ reads, TAO: 99.8% reads",
  },
  {
    op: "Write",
    pct: 9,
    desc: "Updates/inserts",
    source: "TAO: ~0.1% writes, relaxed for testing",
  },
...
const printSizeDistribution = (dist) => {
  const table = new Table({
    head: ["Tier", "Items", "Items %", "Total Size", "Size %"],
    style: { head: ["cyan"] },
  });

  // Map labels to tier names / 将标签映射到层级名称
  const tierNames = {
    "<100B": "Tiny Metadata",
    "100B-1KB": "Small Structs",
    "1-10KB": "Medium Content",
    "10-100KB": "Large Objects",
    ">=100KB": "Huge Blobs",
  };

  for (const b of dist) {
    if (b.count > 0) {
      const tierName = tierNames[b.label] || b.label;
...
const printConsoleTable = (results) => {
  ...
  const table = new Table({
    head: ["Library", "Hit Rate", "Effective OPS (M/s)", "Perf %", "Memory (MB)"],
    style: { head: ["cyan"] },
  });
...
const genMdEn = (data, sys) => {
  ...
  for (const r of data.results) {
    const perfPct = ((r.effective_ops / best.effective_ops) * 100).toFixed(0);
    lines.push(
      `| ${r.lib} | ${(r.hit_rate * 100).toFixed(2)}% | ${(r.effective_ops / 1e6).toFixed(2)}M/s | ${perfPct}% | ${r.memory_mb.toFixed(3)}MB |`,
    );
  }
  lines.push("");
  ...
  lines.push(
    `Memory: ${fmtSize(cfg.mem_budget)} · Zipf s=${cfg.zipf_s} · R/W/D: ${cfg.read_ratio}/${cfg.write_ratio}/${cfg.delete_ratio}% · Miss: ${cfg.real_miss_ratio}% · Ops: ${(cfg.ops_per_loop / 1e6).toFixed(0)}M×${cfg.loops}\n`,
  );
  ...
  lines.push(
    `Miss penalty: ${data.miss_latency_ns.toLocaleString()}ns — measured via ${data.miss_latency_method}\n`,
  );
  lines.push("");
...
const genMdZh = (data, sys) => {
  ...
  lines.push("### 配置\n");
  lines.push(
    `内存: ${fmtSize(cfg.mem_budget)} · Zipf s=${cfg.zipf_s} · 读/写/删: ${cfg.read_ratio}/${cfg.write_ratio}/${cfg.delete_ratio}% · 未命中: ${cfg.real_miss_ratio}% · 操作: ${(cfg.ops_per_loop / 1e6).toFixed(0)}M×${cfg.loops}\n`,
  );
  ...
  lines.push(
    `未命中惩罚: ${data.miss_latency_ns.toLocaleString()}ns — 通过 ${data.miss_latency_method} 实测\n`,
  );
  lines.push("");
