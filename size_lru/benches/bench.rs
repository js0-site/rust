...
fn calculate_scale_factor(lib_name: &str, target_mb: f64, previous_results: &[BenchResult]) -> f64 {
  if let Some(prev_result) = previous_results.iter().find(|r| r.lib == lib_name) {
    ...
    println!("  {lib_name}: prev_memory={:.1}MB, target={target_mb}MB, raw_scale={raw_scale:.3}, adjusted_scale={adjusted_scale:.3}", prev_result.memory_mb);
    adjusted_scale
  } else {
    ...
    "size_lru" => {
      if let Some(results) = previous_results {
        if let Some(prev_result) = results.iter().find(|r| r.lib == lib) {
          ...
          println!("  {lib}: prev_capacity={}, scaled={scaled_capacity}", prev_result.capacity);
          scaled_capacity
        } else {
          55579991 // Optimized default capacity for ~64MB
        }
      } else {
        55579991 // Optimized default capacity for ~64MB
      }
    ...
    "moka" => {
      if let Some(results) = previous_results {
        if let Some(prev_result) = results.iter().find(|r| r.lib == lib) {
          ...
          52769659 // Optimized default capacity for ~64MB
        }
      } else {
        52769659 // Optimized default capacity for ~64MB
      }
    ...
    "mini_moka" => {
      if let Some(results) = previous_results {
        if let Some(prev_result) = results.iter().find(|r| r.lib == lib) {
          ...
          59469756 // Optimized default capacity for ~64MB
        }
      } else {
        59469756 // Optimized default capacity for ~64MB
      }
    ...
    "hashlink" => {
      if let Some(results) = previous_results {
        if let Some(prev_result) = results.iter().find(|r| r.lib == lib) {
          ...
          124429 // Optimized default capacity for ~64MB
        }
      } else {
        124429 // Optimized default capacity for ~64MB
      }
    ...
    "clru" => {
      if let Some(results) = previous_results {
        if let Some(prev_result) = results.iter().find(|r| r.lib == lib) {
          ...
          54975530 // Optimized default capacity for ~64MB
        }
      } else {
        54975530 // Optimized default capacity for ~64MB
      }
    ...
    "schnellru" => {
      if let Some(results) = previous_results {
        if let Some(prev_result) = results.iter().find(|r| r.lib == lib) {
          ...
          141162 // Optimized default capacity for ~64MB
        }
      } else {
        141162 // Optimized default capacity for ~64MB
      }
...
impl OpSequence {
  ...
  fn new(data_len: usize, config: &BenchConfig, generator: &mut ZipfDataGenerator, ops_count: usize) -> Self {
