...
function printConsoleTable(data) {
  const table = new Table({
    head: ["Library", "FPP", "Contains (M/s)", "Add (M/s)", "Remove (M/s)", "Memory (KB)"],
    style: { head: ["cyan"] },
  });
...
function genMdEn(data, sys) {
  ...
  lines.push(`Test: ${data.items} items, capacity=${data.capacity}\n`);
  ...
  lines.push("| Library | FPP | Contains (M/s) | Add (M/s) | Remove (M/s) | Memory (KB) |");
  lines.push("|---------|-----|----------------|-----------|--------------|-------------|");
...
function genMdZh(data, sys) {
  ...
  lines.push(`测试：${data.items} 条数据，容量=${data.capacity}\n`);
  ...
  // 性能表格
  lines.push("### 性能对比\n");
  lines.push("| 库 | 误判率 | 查询 (百万/秒) | 添加 (百万/秒) | 删除 (百万/秒) | 内存 (KB) |");
  lines.push("|---------|-----|----------------|-----------|--------------|-------------|");
