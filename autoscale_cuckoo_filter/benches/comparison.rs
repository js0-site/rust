...
impl Filter for autoscale_cuckoo_filter::CuckooFilter<[u8], RapidHasher<'_>> {
  fn add_key(&mut self, key: &[u8]) {
...
fn bench_remove(c: &mut Criterion) {
  ...
  g.bench_function("autoscale_cuckoo_filter", |b| {
    b.iter_batched(
      ...
      |mut f: autoscale_cuckoo_filter::CuckooFilter<u64, RapidHasher>| {
        for i in 0..50u64 {
          f.remove(&i);
        }
      },
