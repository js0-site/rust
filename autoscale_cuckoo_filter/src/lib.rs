...
#[cfg(feature = "gxhash")]
pub use cuckoo_filter::GxHasher;
#[cfg(feature = "museair")]
pub use cuckoo_filter::MuseAirHasher;
...
/// Compute hash for u64 directly without trait dispatch.
/// 直接计算 u64 哈希值，避免 trait 分发开销
#[inline(always)]
pub(crate) fn hash_u64<H: Hasher + Clone>(hasher: &H, val: u64) -> u64 {
  let mut h = hasher.clone();
  h.write_u64(val);
  h.finish()
}
