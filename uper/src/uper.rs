...
fn tar_zstd_to_dir<P1: AsRef<Path>, P2: AsRef<Path>>(path: P1, to_dir: P2) -> io::Result<()> {
...
impl Uper {
  pub async fn load(
    ...
    let tmpdir = env::temp_dir().join(format!("uper/{project}/{}", ver_url_li.ver));
  ...
  pub async fn join(self, pk: [u8; 32]) -> Result<()> {
    let recv = self.recv;
    let path = &self.path;
    ...
    if let Ok(size) = xerr::ok!(recv.recv().await) {
        ...
        && let Some(verfiy_dir) = upgrade_verify::check(vb::e_li(ver.0), path, pk)?
      {
        ...
        xerr::log!(fs::remove_dir_all(verfiy_dir));

        pbar.finish_and_clear();

        let exe = path.join(format!("{}{}", self.project, env::consts::EXE_SUFFIX));

        match Command::new(&exe).args(["-v"]).output() {
          Ok(r) => {
