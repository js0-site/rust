...
syntax_str:
  "[COUNT\_count] [BLOCK\_milliseconds] [CLAIM\_min-idle-time] [NOACK] STREAMS\_key [key ...] id\
  \ [id ...]"
title: XREADGROUP
---
...
Within a consumer group, a given consumer (that is, just a client consuming messages from the stream), has to identify with a unique _consumer name_. Which is just a string.
...
- [Null reply](../../develop/reference/protocol-speculls): if the _BLOCK_ option is given and a timeout occurs, or if there is no stream that can be served.
