...
impl msgq::Parse for MailParser {
  ...
  async fn on_error(&self, kv: Kv, error: String) -> Void {
    let n = if !kv.is_empty() {
      String::from_utf8(kv[0].1.to_vec()).ok().and_then(|s| s.parse::<u64>().ok())
    } else {
      None
    };
...
async fn test_async() -> Void {
  ...
  info!("Test results: success={} error={} retry={}", success, error, retry);
  ...
  assert!(retry >= 3, "Expected at least 3 retry attempts for n=1, got {}", retry);
