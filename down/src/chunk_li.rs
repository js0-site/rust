...
#[derive(Clone)]
pub struct ChunkLi {
  pub li: Arc<Mutex<IndexSet<Chunk, RandomState>>>,
  pub size: u64,
}
...
impl ChunkLi {
  ...
  pub fn new(size: u64) -> Self {
    ...
    Self {
      size,
      li: Arc::new(Mutex::new(IndexSet::<_, RandomState>::from_iter(
        (0..total_chunk).map(Chunk::new),
      ))),
    }
  }
}
