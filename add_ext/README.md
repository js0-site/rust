...
**Note:** Unlike the standard library's `Path::with_extension()` which _replaces_ the existing extension, `add_ext` _appends_ the new extension while preserving the original one.
...
**注意：** 与标准库的 `Path::with_extension()` *替换*现有扩展名不同，`add_ext` 会*追加*新扩展名并保留原始扩展名。
