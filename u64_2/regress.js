...
const colW = 14;
console.log(`${"Function".padEnd(colW)} ${"Time".padStart(12)} ${"Mops".padStart(12)} ${"Change".padStart(12)}`);
console.log("-".repeat(colW + 38));

for (const b of BENCHMARKS) {
  const curr = entry[b.name];
  const prevVal = prev ? prev[b.name] : null;
  console.log(`${b.label.padEnd(colW)} ${formatTime(curr).padStart(12)} ${formatMops(curr).padStart(12)} ${formatDiff(curr, prevVal)}`);
}
