...
const genMdEn = (data, sys) => {
  ...
  lines.push(
    `Comparing u64_2 (pair encoding) with vb (varint) using ${data_count.toLocaleString()} integers (mixed distribution: 60% small, 30% medium, 10% large).\n`,
  );
...
const genMdZh = (data, sys) => {
  ...
  lines.push(
    `对比 u64_2（双值编码）与 vb（变长编码），测试数据：${data_count.toLocaleString()} 个整数（混合分布：60% 小值，30% 中值，10% 大值）。\n`,
  );
