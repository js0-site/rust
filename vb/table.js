...
const printConsoleTable = (results, dataCount) => {
  ...
  console.log(`\nVByte Benchmark Results (${dataCount.toLocaleString()} integers):`);
...
const genMdEn = (data, sys) => {
  ...
  lines.push("## VByte Encoding Benchmark\n");
  lines.push(
    `Comparing varint encoding libraries with ${data_count.toLocaleString()} integers (mixed distribution: 60% small, 30% medium, 10% large).\n`,
  );
  ...
  lines.push(
    `${sys.osName} (${sys.arch}) · ${sys.cpu} · ${sys.cores} cores · ${sys.mem}GB · ${sys.rustVer}\n`,
  );
...
const genMdZh = (data, sys) => {
  ...
  lines.push("## VByte 编码性能评测\n");
  lines.push(
    `对比 varint 编码库，测试数据：${data_count.toLocaleString()} 个整数（混合分布：60% 小值，30% 中值，10% 大值）。\n`,
  );
  ...
  lines.push("### 环境\n");
  lines.push(
    `${sys.osName} (${sys.arch}) · ${sys.cpu} · ${sys.cores} 核 · ${sys.mem}GB · ${sys.rustVer}\n`,
  );
