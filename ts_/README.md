...
# ts_ : High-performance, lightweight time measurement for Rust
...
*   **Simplicity**: Exposes a straightforward API with just `nano()` and `sec()` functions.
...
Here is a simple example demonstrating how to retrieve the current time in nanoseconds and seconds:
...
*   **极简易用**：仅暴露 `nano()` 和 `sec()` 两个核心函数，上手即用。
