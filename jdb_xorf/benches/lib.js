...
export const parseCriterion = (target_dir) => {
  ...
  if (existsSync(rates_dir)) {
    ...
    for (const file of rate_files) {
      ...
      if (type === "fp") {
        results[lib][filter][size].false_positive = { rate: parseFloat(content) };
      } else if (type === "fn") {
        results[lib][filter][size].false_negative = { rate: parseFloat(content) };
      } else if (type === "mem") {
