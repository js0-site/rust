...
use std::{hint::black_box, mem, time::Duration};
...
mod jdb_impl {
  use jdb_xorf::{Bf8, Bf16, Bf32, Filter};

  use super::{FilterBench, mem};

  pub struct JdbBf8(Bf8);
  pub struct JdbBf16(Bf16);
  pub struct JdbBf32(Bf32);

  impl FilterBench for JdbBf8 {
    const NAME: &'static str = "jdb_Bf8";
    ...
    fn memory_usage(&self) -> usize {
      mem::size_of::<Bf8>() + self.0.fingerprints.len()
    }
  }

  impl FilterBench for JdbBf16 {
    const NAME: &'static str = "jdb_Bf16";
    ...
    fn memory_usage(&self) -> usize {
      mem::size_of::<Bf16>() + self.0.fingerprints.len() * 2
    }
  }

  impl FilterBench for JdbBf32 {
    const NAME: &'static str = "jdb_Bf32";
    ...
    fn memory_usage(&self) -> usize {
      mem::size_of::<Bf32>() + self.0.fingerprints.len() * 4
    }
  }
}
