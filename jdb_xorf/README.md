...
1. **Hashing**: Mix keys using GxHash (or optional MuseAir) or custom hasher.
...
- **Hash Algorithm**: Default GxHash (based on `gxhash` crate), optional `museair` feature, high-quality mixing function `mix64`.
...
- `Bf<T, F, H = DefaultBuildHasher>`: Generic wrapper, uses hasher `H` and filter `F` to handle arbitrary type `T`, with automatic deduplication.
  - `has(&self, key: &Q) -> bool` / `has_hash(&self, hash: u64) -> bool`
  - `contains_batch(&self, keys: &[Q]) -> Vec<bool>` / `contains_batch_vec(&self, keys: &[Q], results: &mut [bool])`
  - `from_iterator<I>(iter: I) -> Self` / `from_hashes(hashes: impl IntoIterator<Item = u64>) -> Self`
  - Implements standard library `FromIterator<T>` and `Default`
...
1. **哈希化**: 使用 GxHash (或可选的 MuseAir) 或自定义哈希器对键进行混淆。
...
- **哈希算法**: 默认 GxHash (基于 `gxhash` crate)，可选特性 `museair`，高质量混淆函数 `mix64`。
...
- `Bf<T, F, H = DefaultBuildHasher>`: 通用包装器，使用哈希器 `H` 与过滤器 `F` 处理任意类型 `T`，具备自动去重功能。
  - `has(&self, key: &Q) -> bool` / `has_hash(&self, hash: u64) -> bool`
  - `contains_batch(&self, keys: &[Q]) -> Vec<bool>` / `contains_batch_vec(&self, keys: &[Q], results: &mut [bool])`
  - `from_iterator<I>(iter: I) -> Self` / `from_hashes(hashes: impl IntoIterator<Item = u64>) -> Self`
  - 实现标准库 `FromIterator<T>` 与 `Default`
...
本项目是 [js0.site ⋅ 重构互联网计划](https://js0.site) 的开源组件。
