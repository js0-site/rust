...
According to Mueller & Lemire's paper [[2]](#references), the success probability of a single construction attempt has a lower bound of **90%**. Empirical data from this library (based on 1000 trials with 100,000 random keys) shows:
- **Single-shot Success Rate**: **98.70%** (987 out of 1000 trials succeeded on the first attempt)
- **Average Attempts**: **1.013**

This implies a single-attempt failure rate of only ![](https://r.latexeasy.com/image.svg?P_{fail}%20\approx%201.3\%25%20=%200.013).
...
**![](https://r.latexeasy.com/image.svg?10^{-1880}) is a value even more astonishingly close to zero than previously estimated.**
...
A study by Google on DRAM reliability showed that approximately **1.3% of machines experience an uncorrectable memory error per year**. The probability of a hardware error occurring during the split-second filter construction is estimated to be around ![](https://r.latexeasy.com/image.svg?10^{-11}).
...
| **Binary Fuse Bf Failure** | ![](https://r.latexeasy.com/image.svg?\approx%2010^{-1880}) | Physically Impossible |
...
**![](https://r.latexeasy.com/image.svg?10^{-1880}) 是一个比之前估算更惊人的、完全可以视为 0 的数值。**
