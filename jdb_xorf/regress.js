...
const formatTable = (current_results, previous_results = null) => {
  ...
  iterateResults(current_results, (lib, filter, size, metrics) => {
    ...
    const buildThroughput = metrics.build?.mean_ns
      ? (parseInt(size) / (metrics.build.mean_ns / 1000000000) / 10000).toFixed(2)
      : "N/A";
    // query: mean_ns 是整个迭代的时间（包含所有 size 次查询），所以用 size / (mean_ns / 10^9) / 10000
    const queryThroughput = metrics.contains?.mean_ns
      ? (parseInt(size) / (metrics.contains.mean_ns / 1000000000) / 10000).toFixed(2)
      : "N/A";
    ...
    dataByLib[lib][filter][size] = {
      ...
      query_ns: metrics.contains?.mean_ns,
  ...
  for (const lib of sortedLibs) {
    ...
    const sortedFilters = Object.keys(dataByLib[lib]).sort((a, b) => {
      ...
      return filterOrder[a] - filterOrder[b];
    });
    ...
    const metrics = [
      ...
      { name: "内存(KB)", key: "memory_kb", isThroughput: false },
      { name: "假阳率", key: "fp_rate", isThroughput: false },
    ];
    ...
    for (const metric of metrics) {
      ...
      for (const col of colKeys) {
        ...
        if (
          ...
          if (prev) {
            if (metric.isBuild && prev.build?.mean_ns) {
              const prevVal = (
                parseInt(col.size) /
                (prev.build.mean_ns / 1000000000) / 10000
              ).toFixed(2);
            ...
            } else if (!metric.isBuild && prev.contains?.mean_ns) {
              const prevVal = (
                parseInt(col.size) /
                (prev.contains.mean_ns / 1000000000) / 10000
              ).toFixed(2);
...
const main = async () => {
  ...
  await $`rm -rf ${target_dir}/criterion/rates`.quiet();
  await $`rm -rf target/criterion/rates`.quiet();

  // Run benchmarks with bench-jdb feature
  // 运行带 bench-jdb 特性的性能测试
  console.log("运行 bench-jdb 特性的性能测试...\n");
  ...
  saveHistory(history);
  console.log(`已保存到历史记录 (${history.length}/${MAX_HISTORY} 条)`);
};
