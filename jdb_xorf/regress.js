...
const formatTable = (current_results, previous_results = null) => {
  ...
  iterateResults(current_results, (lib, filter, size, metrics) => {
    ...
    const queryThroughput = metrics.has?.mean_ns
      ? (parseInt(size) / (metrics.has.mean_ns / 1000000000) / 10000).toFixed(2)
      : "N/A";
    ...
    dataByLib[lib][filter][size] = {
      ...
      memory_kb: metrics.memory?.bytes ? (metrics.memory.bytes / 1024).toFixed(1) : "N/A",
      fp_rate:
        metrics.false_positive?.rate !== undefined ? metrics.false_positive.rate.toFixed(3) : "N/A",
    };
  });
  ...
  // 按大小排序
  const sortedSizes = Array.from(sizes).sort((a, b) => parseInt(a) - parseInt(b));
  ...
  for (const lib of sortedLibs) {
    ...
    for (const metric of metrics) {
      ...
      for (const col of colKeys) {
        ...
        // 如果有历史数据且是吞吐量指标，显示变化
        if (previous_results && metric.isThroughput && col.data.build_ns && col.data.query_ns) {
          ...
          if (prev) {
            if (metric.isBf && prev.build?.mean_ns) {
              ...
              if (prevNum > 0) {
                const change = (((currVal - prevNum) / prevNum) * 100).toFixed(2);
...
const main = async () => {
  ...
  const previous = history.length > 0 ? history[history.length - 1].results : null;
