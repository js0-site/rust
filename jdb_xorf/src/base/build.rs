...
use super::calc::{MAX_CAPACITY, SizeInfo};
use crate::{
  Desc, Error, Fingerprint, Result, hash_of_hash, mix_key, mod3,
};
...
pub fn try_make<T: Fingerprint>(keys: &[u64], max_iter: usize) -> Result<(Desc, Box<[T]>)> {
  ...
  // 初始化指纹数组
  let mut fingerprints: Box<[T]> = crate::make_fp_block(fp_array_len as usize);
  ...
  for iter_n in 0..max_iter {
    ...
    for &hash in reverse_order.iter().take(size) {
      ...
      unsafe {
        ...
        // Detect u8 counter overflow (>= 64 edges wrap around) and trigger retry
        // 检测 u8 计数器溢出（>= 64 条边回绕）并触发重试
        ...
        if unsafe { (*t2count.get_unchecked(index) >> 2) == 1 } {
          ...
          for offset in [1u8, 2u8] {
            ...
            unsafe {
              ...
              *t2count.get_unchecked_mut(other) =
                t2count.get_unchecked(other).wrapping_sub(4);
...
unsafe fn update_buckets(
  ...
  unsafe {
    if add {
      *t2count.get_unchecked_mut(i0) = t2count.get_unchecked(i0).wrapping_add(4);
      *t2count.get_unchecked_mut(i1) = t2count.get_unchecked(i1).wrapping_add(4);
      *t2count.get_unchecked_mut(i2) = t2count.get_unchecked(i2).wrapping_add(4);
    } else {
      *t2count.get_unchecked_mut(i0) = t2count.get_unchecked(i0).wrapping_sub(4);
      *t2count.get_unchecked_mut(i1) = t2count.get_unchecked(i1).wrapping_sub(4);
      *t2count.get_unchecked_mut(i2) = t2count.get_unchecked(i2).wrapping_sub(4);
    }
