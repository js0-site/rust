...
pub fn try_make<T: Fingerprint, I>(keys: I, max_iter: usize) -> Result<(Desc, Box<[T]>)>
where
...
where
  I: ExactSizeIterator<Item = u64> + Clone,
{
  try_make(keys, max_iter).unwrap_or_else(|_| {
    (
      Desc {
        seed: 0,
        seg_len: 4,
