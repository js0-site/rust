...
impl<'a> FilterRef<'a, u64> for BinaryFuse16Ref<'a> {
  ...
  fn from_dma(descriptor: &[u8], fingerprints: &'a [u8]) -> Self {
    ...
    assert!(
      ...
      "Invalid fingerprint buffer provided - len must be a multiple of u16"
    );
...
mod test {
  ...
  #[test]
  #[should_panic(expected = "Invalid fingerprint buffer provided - len must be a multiple of u16")]
