...
use crate::{
  ...
  error::{Error, Result},
  prelude::bfuse::{Desc, parse_bfuse_descriptor, serialize_bfuse_descriptor},
};
...
impl BinaryFuse32 {
  ...
  pub fn try_from_iterator<T>(keys: T) -> Result<Self>
  where
...
impl TryFrom<&[u64]> for BinaryFuse32 {
  ...
  fn try_from(keys: &[u64]) -> Result<Self> {
...
impl<'a> FilterRef<'a, u64> for BinaryFuse32Ref<'a> {
  ...
  fn from_dma(descriptor: &[u8], fingerprints: &'a [u8]) -> Self {
    ...
    assert!(
      ...
      "Invalid fingerprint buffer provided - len must be a multiple of u32"
    );
...
mod test {
  ...
  #[test]
  #[should_panic(expected = "Invalid fingerprint buffer provided - len must be a multiple of u32")]
