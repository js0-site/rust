...
impl Desc {
  pub const DMA_LEN: usize = 20; // 8 + 4 + 4 + 4
}

#[inline]
pub fn parse_bfuse_desc(desc: &[u8]) -> Desc {
  Desc {
    seed: u64::from_le_bytes(desc[0..8].try_into().unwrap()),
    seg_len: u32::from_le_bytes(desc[8..12].try_into().unwrap()),
    seg_len_mask: u32::from_le_bytes(desc[12..16].try_into().unwrap()),
    seg_count_len: u32::from_le_bytes(desc[16..20].try_into().unwrap()),
  }
}

#[inline]
pub fn serialize_bfuse_desc(desc: &Desc, out: &mut [u8]) {
  out[0..8].copy_from_slice(&desc.seed.to_le_bytes());
  out[8..12].copy_from_slice(&desc.seg_len.to_le_bytes());
  out[12..16].copy_from_slice(&desc.seg_len_mask.to_le_bytes());
  out[16..20].copy_from_slice(&desc.seg_count_len.to_le_bytes());
}
