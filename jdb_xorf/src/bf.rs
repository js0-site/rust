...
impl<T, U, H> Bf<T, crate::Base<U>, H>
where
  T: Hash,
  U: crate::hash::Fingerprint,
  H: Hasher + Default,
{
  /// Construct a Bf from a slice of keys, returning Result.
  /// 从键切片构造 Bf，返回 Result。
  pub fn try_from_keys(keys: &[T]) -> crate::Result<Self> {
    Ok(Self {
      filter: crate::Base::try_from_vec(hash_sort_dedup::<H, T>(keys))?,
  ...
  F: Filter<u64>,
{
  ...
  {
    Self {
      filter: F::from(hash_sort_dedup::<H, T>(keys)),
      _phantom: PhantomData,
    }
  }
}
