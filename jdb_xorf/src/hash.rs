...
/// MurmurHash3 fmix64 multiplier constant.
/// 通过模拟退火筛选的高质量哈希常数。
const MIX_C1: u64 = 0xff51_afd7_ed55_8ccd;

/// RapidHash finalization mix implementation (optimized const version)
/// RapidHash 终结混淆实现 (优化 const 版本)
///
/// 这是一个提供优秀雪崩特性的终结步骤。
/// 我们使用单次乘法和异或旋转来保持极高性能。
#[inline(always)]
pub const fn mix64(k: u64) -> u64 {
  let h = k.wrapping_mul(MIX_C1);
  h ^ h.rotate_right(33)
}
