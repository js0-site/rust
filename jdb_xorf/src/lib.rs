...
/// A DMA serializable filter has two components - the "fixed" descriptor and the variable len fingerprints.
...
pub trait DmaSerializable {
  /// The serialized len of the descriptor. Very small and safe to allocate on-stack if needed.
  const DESCRIPTOR_LEN: usize;

  /// Copies the small fixed-len descriptor part of the filter to an output buffer.
