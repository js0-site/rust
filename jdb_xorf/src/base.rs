...
use crate::{
  ...
  prelude::bfuse::{
    Desc, hash_of_hash, mod3, parse_bfuse_desc, seg_len, serialize_bfuse_desc, size_factor,
  },
};
...
impl<T: Fingerprint> Filter<u64> for Bf<T> {
  #[inline(always)]
...
impl<T: Fingerprint> Bf<T> {
  ...
  {
    let (desc, fingerprints) = make(keys, 1000);
    Self { desc, fingerprints }
  }
}

impl<T: Fingerprint> From<&[u64]> for Bf<T> {
  fn from(keys: &[u64]) -> Self {
    Self::from_iterator(keys.iter().copied())
  }
}

impl<T: Fingerprint> From<&Vec<u64>> for Bf<T> {
...
impl<T: Fingerprint> DmaSerializable for Bf<T> {
  const LEN: usize = Desc::DMA_LEN;
...
#[derive(Debug, Clone)]
pub struct BfRef<'a, T: Fingerprint> {
  /// Descriptor
  /// 描述符
  pub desc: Desc,
...
impl<'a, T: Fingerprint> Filter<u64> for BfRef<'a, T> {
  #[inline(always)]
  fn contains(&self, key: &u64) -> bool {
    contains_impl(
      key,
...
impl<'a, T: Fingerprint> FilterRef<'a, u64> for BfRef<'a, T> {
...
fn make<T: Fingerprint, I>(keys: I, max_iter: usize) -> (Desc, Box<[T]>)
where
  I: ExactSizeIterator<Item = u64> + Clone,
{
