  ...
  fn decode_and_query(data: &[u8]) {
    ...
    for key in KEYS {
      if bf_string.has(*key) {
        println!("Filter contains '{key}': YES");
      } else {
        println!("Filter contains '{key}': NO (Error!)");
      }
    }

    let unknown = "durian";
    if bf_string.has(unknown) {
      println!("Filter contains '{unknown}': YES (False Positive)");
    } else {
      println!("Filter contains '{unknown}': NO");
    }
  }
  // 1. 序列化流程 (Encode)
  let encoded_data = create_and_encode();
  let encoded_len = encoded_data.len();
  println!("Encoded size: {encoded_len} bytes");
